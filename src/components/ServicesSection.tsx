...
export default function ServicesSection() {
  ...
  return (
    <section id="xizmatlar" className="relative w-full py-16 sm:py-24 bg-[url('/section3-bg-light.png')] dark:bg-[url('/section3-bg.png')] bg-fixed bg-cover bg-center bg-no-repeat overflow-hidden transition-colors duration-500">
          ...
          {services.map((service, index) => (
            <Link href={service.href} key={service.id} className={`block ${service.colSpan}`}>
              <motion.div
                ...
                className={`group relative flex flex-col p-6 sm:p-8 md:p-10 h-full rounded-[2rem] bg-white/70 dark:bg-black/40 backdrop-blur-2xl border border-black/5 dark:border-white/10 overflow-hidden transition-all duration-500 hover:border-black/10 dark:hover:border-white/20 hover:-translate-y-1 hover:bg-white/90 dark:hover:bg-black/60 shadow-xl dark:shadow-2xl`}
              >
                ...
                <service.icon 
                  className="absolute -bottom-10 -right-10 w-64 h-64 text-black/[0.02] dark:text-white/[0.02] group-hover:text-black/[0.05] dark:group-hover:text-white/[0.05] transition-all duration-700 group-hover:scale-110 group-hover:-rotate-6 pointer-events-none" 
                  strokeWidth={1}
                />
